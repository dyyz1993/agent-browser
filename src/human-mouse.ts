...
function generateBezierPath(start: Point, end: Point): Point[] {
  ...
  return [
    start,
    { x: start.x + dx * 0.3 + Math.cos(angle) * offset, y: start.y + dy * 0.3 + Math.sin(angle) * offset },
    { x: start.x + dx * 0.7 + Math.cos(angle) * offset * 0.5, y: start.y + dy * 0.7 + Math.sin(angle) * offset * 0.5 },
    end,
  ];
}
...
function generatePath(start: Point, end: Point, type: HumanPathType): Point[] {
  switch (type) {
    case 'arc': return generateArcPath(start, end);
    case 'bezier': return generateBezierPath(start, end);
    case 'random': return generateRandomPath(start, end);
...
async function moveAlongPath(page: Page, pts: Point[], steps: number, preset: typeof PRESETS[HumanPathType]) {
...
export async function humanClick(
  ...
  // 点击
  await sleep(rand(20, 50));
  await page.mouse.down({ button: options?.button ?? 'left', clickCount: options?.clickCount ?? 1 });
  await sleep(rand(20, 50));
  ...
  for (let i = 0; i < args.length; i++) {
    ...
    if (arg === '--human' || arg === '-H') {
      ...
      if (next && ['bezier', 'arc', 'random', 'linear'].includes(next)) {
        config.pathType = next as HumanPathType;
        i++;
      }
      config.enabled = true;
      continue;
    ...
    if (lower.startsWith('--hum') && lower !== '--human') {
      console.error(`\n⚠️  "${arg}" not recognized. Did you mean "--human"?\n`);
    }
